...
public class ImageToGlassPipeline : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        // Collect all managers to configure. Use the explicit reference when wired;
        // otherwise discover every FlockManager in the scene so the pipeline works
        // without being co-located on each flock's GameObject.
        FlockManager[] targets;
        if (flockManager != null)
        {
            targets = new[] { flockManager };
        }
        else
        {
            targets = FindObjectsByType<FlockManager>(FindObjectsSortMode.None);
            if (targets.Length == 0)
                Debug.LogWarning("ImageToGlassPipeline: no FlockManagers found in scene.");
        }

        foreach (var mgr in targets)
            mgr.SetWindowConfig(WindowWidth, WindowHeight, fractureSeed);
        ...
        if (shardMaterialTemplate != null)
        {
            ...
            foreach (var mgr in targets)
                mgr.SetShardMaterial(SharedMaterial);
        }
